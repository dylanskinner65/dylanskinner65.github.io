...
export function LiveNhlDashboard() {
	...
	const handleMouseMove = (e: React.MouseEvent<SVGSVGElement, MouseEvent>) => {
		...
		const svgWidth = 800;
		// Map client mouse X back to SVG local X coordinate space
		const clickX = ((e.clientX - rect.left) / rect.width) * svgWidth;

		// Find the nearest data point in the trajectory based on X coordinate
		let nearestIdx = 0;
	...
	const handleChartClick = () => {
		if (hoveredIndex !== null) {
			setClickedIndex(hoveredIndex === clickedIndex ? null : hoveredIndex);
		}
	};
	...
	return (
		<div className="w-full bg-background text-foreground rounded-2xl border border-foreground/5 p-6 md:p-8 shadow-2xl relative overflow-hidden backdrop-blur-md">
					...
					<div className="flex gap-4 overflow-x-auto pb-3 scrollbar-thin">
						{games.map((game) => (
							<button
								...
								className={`p-4 rounded-xl border shrink-0 w-64 text-left transition-all ${
			...
			{gameDetail && selectedGame && (
				<div className="space-y-8">
					...
					<div className="p-6 rounded-2xl bg-foreground/[0.02] border border-foreground/5 relative overflow-hidden shadow-lg shadow-black/10">
						...
						<div className="flex flex-col md:flex-row justify-between items-center gap-6">
							{/* Away Team Details */}
							<div className="flex items-center gap-4 w-full md:w-auto md:flex-1 justify-center md:justify-end font-bold italic">
							...
							{/* Home Team Details */}
							<div className="flex items-center gap-4 w-full md:w-auto md:flex-1 justify-center md:justify-start font-bold italic">
								<span className="text-4xl md:text-5xl font-black">
								...
								(() => {
									...
									return (
										<div className="flex flex-col items-center">
											<span
												className={`text-4xl font-extrabold italic drop-shadow-lg transition-all duration-300 ${getProbabilityColor(
													latestProb,
												)}`}
											>
												{Math.round(displayProb * 100)}%
											</span>
											<span className="text-xs font-bold opacity-60 italic mt-0.5">
					...
					<div className="bg-foreground/[0.01] border border-foreground/5 rounded-2xl p-4 md:p-6 shadow-inner relative">
						...
						<div className="w-full relative">
							{/* biome-ignore lint/a11y/useKeyWithClickEvents: custom interactive SVG timeline chart */}
							<svg
								ref={svgRef}
								viewBox="0 0 800 250"
								className="w-full h-auto cursor-crosshair select-none outline-none"
							...
							>
								...
								<text
									x="12"
									y="24"
									className="fill-foreground/40 font-bold text-[9px]"
								>
									100%
								</text>
								...
								<text
									x="12"
									y="129"
									className="fill-foreground/50 font-black text-[10px]"
								>
									50%
								</text>
								...
								<text
									x="278.3"
									y="32"
									className="fill-foreground/45 font-bold text-[9px] italic"
								>
									END P1
								</text>
						...
						<AnimatePresence>
							{activeCoord && (
								<motion.div
									...
									className="mt-4 p-4 rounded-xl bg-foreground/[0.02] border border-foreground/10 shadow-xl backdrop-blur-xl flex flex-col md:flex-row md:items-center justify-between gap-4"
								>
									<div className="space-y-1 select-none">
										<div className="flex items-center gap-2">
											<span className="px-2 py-0.5 rounded bg-accent/15 text-accent font-black text-xs uppercase tracking-wider italic">
												...
												-{" "}
												{Math.floor(
													((3600 - activeCoord.pt.seconds_remaining) % 1200) /
														60,
												)}
												:
												{String(
										...
										<p className="text-sm opacity-80 leading-relaxed font-semibold italic">
									...
									{(() => {
										...
										return (
											<div className="shrink-0 flex items-center gap-3 bg-background/40 border border-foreground/5 p-2 px-4 rounded-lg">
												<span className="text-xs opacity-50 font-bold italic uppercase tracking-wider">
													{displayTeamAbbrev} chance to win:
												</span>
												<span
													className={`text-xl font-black italic transition-all ${getProbabilityColor(prob)}`}
												>
													{Math.round(displayProb * 100)}%
												</span>
											</div>
					...
					<div className="space-y-3">
						...
						<div className="max-h-[350px] overflow-y-auto border border-foreground/5 rounded-2xl p-2 md:p-3 space-y-4 bg-foreground/[0.01] scrollbar-thin">
							{(() => {
										...
										return (
											<div key={p} className="space-y-2">
												...
												<div
													...
													tabIndex={0}
													role="button"
													className="flex justify-between items-center p-3 rounded-xl bg-background border border-foreground/10 cursor-pointer hover:bg-foreground/[0.03] select-none transition-all sticky top-0 z-10 shadow-lg shadow-black/20 backdrop-blur-md outline-none"
												>
													<div className="flex items-center gap-2 font-bold italic text-sm text-foreground">
														...
														{pEvents.map(({ event, realIdx }) => {
															...
															return (
																<button
																	...
																	className={`w-full p-3.5 rounded-xl border text-left flex items-start justify-between gap-4 transition-all duration-200 select-none ${
																		isSelected
																...
																>
																	<div className="space-y-1.5">
																		<div className="flex items-center gap-2">
																			<span className="text-xs opacity-50 font-bold uppercase tracking-wider italic">
																				{event.period <= 3
																					? `P${event.period}`
																					: event.period === 4
																						? "OT"
																			...
																			{event.event_type === "GOAL" && (
																				<span className="px-2 py-0.5 rounded bg-emerald-500/10 text-emerald-400 text-[10px] font-black uppercase italic tracking-widest animate-pulse">
																					Goal
																				</span>
																			)}
																			{event.event_type === "PENALTY" && (
																				<span className="px-2 py-0.5 rounded bg-red-500/10 text-red-400 text-[10px] font-black uppercase italic tracking-widest">
																					Penalty
																				</span>
																			)}
																		</div>

																		<p className="text-sm font-semibold italic leading-relaxed opacity-90">
																			"{event.description}"
																		</p>
																	</div>
																	...
																	{(() => {
																		...
																		return (
																			<div className="shrink-0 text-right space-y-1 font-bold italic">
																				<span className="text-xs opacity-50">
																					{displayTeamAbbrev} to Win
																				</span>
																				<p
																					className={`text-base font-black transition-all ${getProbabilityColor(prob)}`}
																				>
																					{Math.round(displayProb * 100)}%
																				</p>
																			</div>
