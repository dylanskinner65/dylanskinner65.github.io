							...
							<Command.List className="overflow-y-auto overflow-x-hidden p-2 scroll-smooth">
								...
								{query.length > 0 && results.length > 0 && (
									<Command.Group
										...
										className="px-3 py-2 text-[10px] font-black uppercase tracking-widest text-accent/60 italic"
									>
										{results.slice(0, 8).map(({ item }) => (
											<Command.Item
												...
												onSelect={() =>
													runCommand(() =>
														navigate(
															`/${item.type === "blog" ? "blog" : "projects"}/${item.slug}`,
														),
													)
												}
								...
								<Command.Group
									heading="NAVIGATION"
									className="px-3 py-2 text-[10px] font-black uppercase tracking-widest text-foreground/20 italic"
								>
									<Command.Item
								...
								>
									...
									>
										{resolvedTheme === "dark" ? (
											<Sun className="w-4 h-4 opacity-40 group-aria-selected:opacity-100" />
										) : (
											<Moon className="w-4 h-4 opacity-40 group-aria-selected:opacity-100" />
										)}
										<span className="text-sm font-medium">
											Toggle {resolvedTheme === "dark" ? "Light" : "Dark"} Mode
										</span>
										<span className="ml-auto text-[10px] font-sans px-2 py-0.5 rounded-full bg-foreground/5 opacity-40">
											T
										</span>
									</Command.Item>
									<Command.Item
									...
									<Command.Item
										onSelect={() =>
											runCommand(() => {
												window.open(
													"/Dylan Skinner - Resume (Website - 21 Jan).pdf",
													"_blank",
												);
											})
										}
								...
								<div className="flex items-center gap-4 text-[8px] font-bold opacity-40">
									<div className="flex items-center gap-3">
										<div className="flex items-center gap-1">
											<span className="border border-foreground/10 px-1.5 py-0.5 rounded bg-background">
												↑↓
											</span>
											<span>NAVIGATE</span>
										</div>
