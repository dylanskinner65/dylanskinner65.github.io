...
const fixPath = (src: string) => {
	...
	if (
		(cleanSrc.includes("blog_files/") ||
			cleanSrc.includes("projects_files/")) &&
		!cleanSrc.startsWith("/")
	) {
		cleanSrc = `/${cleanSrc}`;
	}
			...
			if (domNode.type === "tag" || domNode.name) {
					...
					const blocks = domNode.children
						...
						.map((pre: any) => {
							const codeNode =
								...
								pre.children[0];
							return {
								lang: pre.attribs["data-lang"] || "javascript",
								label: pre.attribs["data-label"] || "JavaScript",
				...
				if (domNode.name === "li") {
					return (
						<li className="text-lg md:text-xl font-light italic opacity-70 leading-relaxed flex gap-6">
							<span className="text-accent font-black not-italic shrink-0 mt-1">
								•
							</span>
				...
				if (domNode.name === "th") {
					return (
						<th
							{...domNode.attribs}
							className="p-6 border border-foreground/10 bg-emerald-900 text-white dark:bg-emerald-500 dark:text-emerald-950 uppercase text-[10px] font-black tracking-widest"
						>
							{domToReact(domNode.children, options)}
						</th>
					);
				...
				if (domNode.name === "td") {
					return (
						<td
							{...domNode.attribs}
							className="p-6 border border-foreground/10 text-lg font-light italic opacity-80 transition-opacity"
						>
							{domToReact(domNode.children, options)}
						</td>
					);
