...
(
	($) => {
		$.extend({
			tablesorter: new (function () {
				...
				function buildParserCache(table, $headers) {
					...
					if (rows[0]) {
						...
						for (var i = 0; i < l; i++) {
							...
							if ($.metadata && $($headers[i]).metadata()?.sorter) {
								p = getParserById($($headers[i]).metadata().sorter);
							} else if (table.config.headers[i]?.sorter) {
				...
				function buildCache(table) {
					...
					var totalRows = table.tBodies[0]?.rows.length || 0,
						totalCells = table.tBodies[0].rows[0]?.cells.length || 0,
				...
				function _checkCellColSpan(table, _rows, row) {
					var arr = [],
						r = table.tHead.rows,
						c = r[row].cells;
					...
					for (var i = 0; i < c.length; i++) {
						...
						if (cell.colSpan > 1) {
							arr = arr.concat(_checkCellColSpan(table, headerArr, row++));
						} else {
				...
				function setHeadersCss(_table, $headers, list, css) {
					// remove all header information
					$headers.removeClass(css[0]).removeClass(css[1]);

					var h = [];
					$headers.each(function (_offset) {
						if (!this.sortDisabled) {
							h[this.column] = $(this);
						}
					});
				...
				function fixColumnWidth(table, _$headers) {
					var c = table.config;
					if (c.widthFixed) {
				...
				function _makeSortText(i) {
					return `((a[${i}] < b[${i}]) ? -1 : ((a[${i}] > b[${i}]) ? 1 : 0));`;
				}

				function _makeSortTextDesc(i) {
					return `((b[${i}] < a[${i}]) ? -1 : ((b[${i}] > a[${i}]) ? 1 : 0));`;
				}

				function _makeSortNumeric(i) {
					return `a[${i}]-b[${i}];`;
				}

				function _makeSortNumericDesc(i) {
					return `b[${i}]-a[${i}];`;
				}

				function _sortText(a, b) {
				...
				function _getCachedSortType(parsers, i) {
				...
				this.construct = function (settings) {
					return this.each(function () {
						...
						$headers
							.click(function (e) {
								var totalRows = $this[0].tBodies[0]?.rows.length || 0;
						...
						$this
							...
							.bind("updateCell", function (_e, cell) {
							...
							.bind("applyWidgetId", function (_e, id) {
						...
						if ($.metadata && $(this).metadata()?.sortlist) {
		...
		ts.addParser({
			...
			is: (s) => /^[£$€?.]/.test(s),
			format: (s) =>
				$.tablesorter.formatFloat(s.replace(new RegExp(/[£$€]/g), "")),
			type: "numeric",
		});
		...
		ts.addParser({
			...
			format: (s) =>
				jQuery.trim(s.replace(new RegExp(/(https?|ftp|file):\/\//), "")),
			type: "text",
		});
		...
		ts.addParser({
			id: "time",
			is: (s) =>
				/^(([0-2]?[0-9]:[0-5][0-9])|([0-1]?[0-9]:[0-5][0-9]\s(am|pm)))$/.test(
					s,
				),
			format: (s) =>
				$.tablesorter.formatFloat(new Date(`2000/01/01 ${s}`).getTime()),
			type: "numeric",
		});
		ts.addParser({
			id: "metadata",
			is: (_s) => false,
			format: (_s, table, cell) => {
				var c = table.config,
		...
		ts.addWidget({
			...
			format: (table) => {
				...
				$("tr:visible", table.tBodies[0]).each(function (_i) {
					$tr = $(this);
